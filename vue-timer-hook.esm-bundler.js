/*!
  ...
  * @license MIT
  */
import { computed, ref, onUnmounted, reactive, toRef } from 'vue';
...
class Time {
    ...
    static getSecondsFromExpiry(expiry, shouldRound) {
        ...
        if (milliSecondsDistance > 0) {
            ...
            return ref(shouldRound ? Math.round(val) : val);
        }
        return ref(0);
    }
    ...
    static getSecondsFromTimeNow() {
        ...
        return ref(currentTimestamp / 1000 - offset);
    }
...
const useStopwatch = (offsetTimestamp = 60, autoStart = true) => {
    let interval;
    const passedSeconds = ref(Time.getSecondsFromExpiry(offsetTimestamp, true) || 0);
    const prevTime = ref(epochSeconds());
    const seconds = ref(passedSeconds.value +
